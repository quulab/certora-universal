...
// `executionId` must be clean when not in the middle of a call
...
// `hasRole()` is consistent with `getAccess()`
...
// `canCall()`, `getAccess()`, `hasRole()`, `isTargetClosed()` and `getTargetFunctionRole()` do NOT revert
rule noRevert(env e) {
...
// Admin restrictions are correct
...
// `getAccess()` works as expected on time change
...
// `getAccess()` works as expected on arbitrary function call
...
// `isTargetClosed()` works as expected on time change
...
// `isTargetClosed()` works as expected on arbitrary function call
...
// `getTargetFunctionRole()` works as expected on time change
...
// `getTargetFunctionRole()` works as expected on arbitrary function call
...
// `getTargetAdminDelay()` works as expected on time change
...
// `getTargetAdminDelay()` works as expected on arbitrary function call
...
// `getRoleGrantDelay()` works as expected on time change
...
// `getRoleGrantDelay()` works as expected on arbitrary function call
...
// `getRoleAdmin()` works as expected on arbitrary function call
...
// `getRoleGuardian()` works as expected on arbitrary function call
...
// `getNonce()` works as expected on arbitrary function call
...
// `getSchedule()` works as expected on time change
...
// `getSchedule()` works as expected on arbitrary function call
...
// Restricted functions can only be called by owner
...
// `canCallExtended()` works as expected
...
// `grantRole()` updates storage as expected
...
// `revokeRole()` updates storage as expected
