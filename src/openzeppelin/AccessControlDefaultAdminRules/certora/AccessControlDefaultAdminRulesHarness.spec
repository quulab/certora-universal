...
// `defaultAdmin` holds the `DEFAULT_ADMIN_ROLE`
...
// `DEFAULT_ADMIN_ROLE`'s admin is always `DEFAULT_ADMIN_ROLE`
invariant defaultAdminRoleAdminConsistency()
  getRoleAdmin(DEFAULT_ADMIN_ROLE()) == DEFAULT_ADMIN_ROLE();

// Owner is the `defaultAdmin`.
// Notice: writing this as an invariant would be flagged by Certora as trivial. Writing it as a rule is just as valid: we
// verify the is true for any state of the storage.
rule ownerConsistency() {
    env e;
    assert defaultAdmin() == owner(e);
}
...
rule onlyGrantCanGrant(env e, method f, bytes32 role, address account) filtered {
...
// `defaultAdmin` is only affected by accepting an admin transfer or renouncing
...
// `pendingDefaultAdmin` is only affected by beginning, completing (accept or renounce), or canceling an admin transfer  
...
// `defaultAdminDelay` can't be changed atomically by any function
...
// `pendingDefaultAdminDelay` is only affected by `changeDefaultAdminDelay()` or `rollbackDefaultAdminDelay()`
...
// `defaultAdminDelayIncreaseWait` can't be changed atomically by any function
...
// A default admin can't change in less than the applied schedule
...
// A delay can't change in less than the applied schedule
...
// `beginDefaultAdminTransfer()` sets a pending default admin and its schedule
...
// `acceptDefaultAdminTransfer()` updates defaultAdmin resetting the pending admin and its schedule
...
// `cancelDefaultAdminTransfer()` resets pending default admin and its schedule
...
// `changeDefaultAdminDelay()` sets a pending default admin delay and its schedule
...
// Pending delay wait is set depending on increasing or decreasing the delay
...
// `rollbackDefaultAdminDelay()` resets the delay and its schedule
